...
def main(args):
    ...
    for file1 in dir.iterdir():
        if not file1:
            continue
        outname = save / Path(file1.stem[:len(file1.stem)])
        output = subprocess.run(["qualimap", "bamqc", "-bam", str(file1), "-outdir", str(outname),
                                 "-nt", str(args.threads)], capture_output=True)
