...
def main(args):
    ...
    if not all(x.name.endswith(".sam") for x in dir.iterdir()):
        raise FileExistsError(
            f"Give files are not *.sam: {list(filter(lambda x: not x.name.endswith('.sam'), list(dir.iterdir())))}")
    ...
    for file1 in dir.iterdir():
        ...
        output = subprocess.run(["samtools", "view", "-S", "-b", str(file1), "-o", str(outname),
                                 "--threads", str(args.threads)], capture_output=True)
